...
class ProductController extends Controller
{
    ...
    public function viewproduct($product_id) {
        if (DB::table('loans')->where('product_id', $product_id)->exists()) {
            $product = DB::select('select products.*, users.name as owner, loans.deadline as deadline from products, users, loans where products.id = ? and loans.product_id = ? and users.id = products.owner_id',[$product_id, $product_id])[0];
    ...
    public function browse($id) {
        $products = DB::select('select products.*, users.name as owner from products, users where products.owner_id = users.id and users.id != ?',[$id]);
